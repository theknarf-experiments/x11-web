...
//
// The `clippy::` entries below are not style opinions we are ducking: the
// body of this module is `gl_generator`'s output, regenerated by build.rs on
// every clean build, so there is no source file to fix. Its emitted shape
// trips three lints — `missing_transmute_annotations` (~1048×: every GL entry
// point is a `transmute(self.storage)` with the fn type only on the binding),
// `unused_unit` (~1002×: void-returning commands are written `-> ()`), and
// `upper_case_acronyms` (4×: the `GLDEBUGPROC*` callback typedefs, whose names
// are fixed by the OpenGL headers). Without these, `cargo clippy -- -D
// warnings` cannot be run over the workspace at all, which is what actually
// costs us: 2054 unfixable findings drown the 16 real ones.
#[allow(
