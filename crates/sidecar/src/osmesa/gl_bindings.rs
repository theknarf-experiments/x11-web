...
use super::{fns, is_available, GL_FALSE, GL_TRUE};
...
pub fn gl_get_pixel_mapuiv(map: u32, values: &mut [u32]) {
    unsafe {
        (fns().get_pixel_mapuiv)(map, values.as_mut_ptr());
    }
}

pub fn gl_get_pixel_mapusv(map: u32, values: &mut [u16]) {
    unsafe {
        (fns().get_pixel_mapusv)(map, values.as_mut_ptr());
    }
}

pub fn gl_get_mapdv(target: u32, query: u32, v: &mut [f64]) {
    unsafe {
        (fns().get_mapdv)(target, query, v.as_mut_ptr());
    }
}

pub fn gl_get_mapfv(target: u32, query: u32, v: &mut [f32]) {
    unsafe {
        (fns().get_mapfv)(target, query, v.as_mut_ptr());
    }
}

pub fn gl_get_mapiv(target: u32, query: u32, v: &mut [i32]) {
    unsafe {
        (fns().get_mapiv)(target, query, v.as_mut_ptr());
    }
}
...
pub fn gl_get_shaderiv(shader: u32, pname: u32, params: &mut [i32]) {
    if let Some(f) = fns().get_shaderiv {
        unsafe {
            f(shader, pname, params.as_mut_ptr());
        }
    }
}
...
pub fn gl_get_programiv(program: u32, pname: u32, params: &mut [i32]) {
    if let Some(f) = fns().get_programiv {
        unsafe {
            f(program, pname, params.as_mut_ptr());
        }
    }
}
    ...
    if let Some(f) = fns().uniform_matrix4fv {
        unsafe {
            f(
                location,
                count,
                if transpose { GL_TRUE } else { GL_FALSE },
                value.as_ptr(),
            );
        }
    }
...
pub fn gl_delete_buffers(buffers: &[u32]) {
    if let Some(f) = fns().delete_buffers {
        unsafe {
            f(buffers.len() as i32, buffers.as_ptr());
        }
    }
}
...
pub fn gl_renderbuffer_storage(target: u32, internal_format: u32, width: i32, height: i32) {
    if let Some(f) = fns().renderbuffer_storage {
        unsafe {
            f(target, internal_format, width, height);
        }
    }
}
...
pub fn gl_delete_vertex_arrays(arrays: &[u32]) {
    if let Some(f) = fns().delete_vertex_arrays {
        unsafe {
            f(arrays.len() as i32, arrays.as_ptr());
        }
    }
}
