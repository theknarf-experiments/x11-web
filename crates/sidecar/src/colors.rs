...
/// Build the color database.  Tries the system file first, then falls back to
...
fn build_db() -> HashMap<String, (u16, u16, u16)> {
    ...
    let mut map = HashMap::with_capacity(BUILTIN_COLORS.len());
    for &(name, r, g, b) in BUILTIN_COLORS.iter() {
        let key = normalize(name);
        map.entry(key).or_insert((scale8(r), scale8(g), scale8(b)));
    }
    map
...
/// Complete built-in X11 color database (all 753 entries from the standard
/// `/usr/share/X11/rgb.txt`).  Used as a fallback when the system file is not
/// available.
///
/// Each entry is `(name, r8, g8, b8)` with 8-bit components that are scaled to
/// 16-bit at database-build time.
static BUILTIN_COLORS: &[(&str, u8, u8, u8)] = &[
    ("snow", 255, 250, 250),
    ...
    ("gray100", 255, 255, 255),
    ("grey100", 255, 255, 255),
    // --- Additional standard entries at the end of rgb.txt ---
