...
/// 3: SelectCursorInput
pub(crate) fn handle_select_cursor_input(state: &mut ClientState, data: &[u8], _seq: u16) -> Vec<u8> {
    if data.len() >= 12 {
...
pub(crate) fn handle_get_cursor_image_and_name(state: &mut ClientState, data: &[u8], seq: u16) -> Vec<u8> {
    let _ = data;
    let cursor_id = state.current_cursor;
...
pub(crate) fn handle_change_cursor_by_name(state: &mut ClientState, data: &[u8], _seq: u16) -> Vec<u8> {
    if data.len() >= 12 {
        let source_cursor = state.read_u32(data, 4);
        let name_len = state.read_u16(data, 8) as usize;
        if data.len() >= 12 + name_len {
