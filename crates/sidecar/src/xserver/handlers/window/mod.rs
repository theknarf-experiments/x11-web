...
pub(crate) use create::{handle_create_window, handle_destroy_window, handle_destroy_subwindows};
pub(crate) use attributes::{handle_change_window_attributes, handle_get_window_attributes, handle_change_save_set};
pub(crate) use map::{handle_map_window, handle_map_subwindows, handle_unmap_window, handle_unmap_subwindows};
pub(crate) use configure::{handle_reparent_window, handle_configure_window, handle_circulate_window};
...
pub(crate) fn update_sibling_visibility(state: &mut ClientState, wid: u32, seq: u16, msb_first: bool) {
    ...
    let save_under_active = state.windows.get(&wid)
        .map(|w| w.save_under)
        .unwrap_or(false);
    ...
    for &sib_id in &siblings {
        let old_vis = state.windows.get(&sib_id).map(|w| w.visibility).unwrap_or(0);
        ...
        if new_vis != old_vis {
            ...
            if old_vis > new_vis && !save_under_active {
                let has_backing = state.windows.get(&sib_id)
                    .map(|w| w.backing_store > 0)
                    .unwrap_or(false);

                if !has_backing {
                    let (sib_w, sib_h, sib_mask) = state.windows.get(&sib_id)
