//! Wayland sidecar's metric instruments. SDK init lives in
//! `x11-web-telemetry`; this module defines the `Metrics` bag the
//! sidecar records on, plus a thin `init()` wrapper that names this
//! binary's service.
//!
//! Verbatim copy of `crates/sidecar/src/telemetry.rs` apart from
//! `SERVICE_NAME` — the instrument names are deliberately identical
//! so a dashboard can group the two sidecars by `service.name`
//! without needing per-sidecar metric names.
...
const SERVICE_NAME: &str = "x11-web-sidecar-wayland";
...
pub struct Metrics {
    ...
    /// Display updates pushed to the backend (PutImage / window
    /// lifecycle). The byte volume on the wire is covered by the
    /// backend's `frame_bytes`.
