...
test.describe("XSETTINGS manager", () => {
	...
	test("XSETTINGS_SETTINGS property is set in binary format", async ({
		...
		const result = await runPythonScript(
			sidecarContainer,
			"xsettings_settings_binary_format.py",
			{ env: { DISPLAY: ":99" } },
		);
...
test.describe
	.serial("EWMH compliance for real applications", () => {
		...
		test("XSETTINGS manager is running", async ({ sidecarContainer }) => {
			const output = (
				await runPythonScript(
					sidecarContainer,
					"xsettings_manager_is_running.py",
					{ env: { DISPLAY: ":99" } },
				)
			).output.trim();
