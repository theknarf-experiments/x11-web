...
test.describe
	.serial("RENDER extension operations", () => {
...
test.describe
	.serial("RENDER CreatePicture validation", () => {
		test("CreatePicture rejects invalid drawable", async ({
			...
			const output = (
				await runPythonScript(
					sidecarContainer,
					"createpicture_rejects_invalid_drawable.py",
					{ env: { DISPLAY: ":99" } },
				)
			).output.trim();
		...
		test("CreatePicture validates format-depth compatibility", async ({
			...
			const output = (
				await runPythonScript(
					sidecarContainer,
					"createpicture_validates_format_depth_compatibility.py",
					{ env: { DISPLAY: ":99" } },
				)
			).output.trim();
			expect(output).toContain("render_present=True");
...
test.describe
	.serial("rendercheck conformance", () => {
...
test.describe
	.serial("rendercheck comprehensive", () => {
		test.setTimeout(300_000);
...
test.describe("rendercheck comprehensive", () => {
	...
	test("rendercheck per-category breakdown all pass", async ({
		...
		for (const cat of categories) {
			...
			console.log(
				`rendercheck ${cat}: ${failed === 0 ? "PASS" : `${failed} FAILED`}`,
			);
...
test.describe
	.serial("Extension presence verification", () => {
		...
		test("RENDER extension version is correct", async ({
			...
			const output = (
				await runPythonScript(
					sidecarContainer,
					"render_extension_version_is_correct.py",
					{ env: { DISPLAY: ":99" } },
				)
			).output.trim();
			expect(output).toContain("render_present=True");
...
test.describe
	.serial("RENDER extension (Phase 7)", () => {
		test("QueryPictFormats returns ARGB32, RGB24, A8, A1", async ({
			...
			const output = (
				await runPythonScript(
					sidecarContainer,
					"querypictformats_returns_argb32_rgb24_a8_a1.py",
					{ env: { DISPLAY: ":99" } },
				)
			).output.trim();
			expect(output).toContain("render_present=True");
...
test.describe
	.serial("rendercheck full coverage", () => {
		for (const category of [
			"fill",
			"dcomp",
			"scomp",
...
test.describe
	.serial("Host access control compliance", () => {
		test("xhost reports access control state", async ({ sidecarContainer }) => {
			const output = await execInSidecar(
				sidecarContainer,
				"xhost 2>&1 || true",
			);
		...
		test("ListHosts returns valid response via python3", async ({
			...
			const output = (
				await runPythonScript(
					sidecarContainer,
					"listhosts_returns_valid_response_via_python3.py",
					{ env: { DISPLAY: ":99" } },
				)
			).output.trim();
		...
		test("Composite extension: QueryVersion and RedirectWindow", async ({
			...
			const output = (
				await runPythonScript(
					sidecarContainer,
					"composite_extension_queryversion_and_redirectwindow.py",
					{ env: { DISPLAY: ":99" } },
				)
			).output.trim();
		...
		test("DAMAGE extension: DamageCreate and DamageDestroy", async ({
			...
			const output = (
				await runPythonScript(
					sidecarContainer,
					"damage_extension_damagecreate_and_damagedestroy.py",
					{ env: { DISPLAY: ":99" } },
				)
			).output.trim();
		...
		test("Present extension: QueryVersion and QueryCapabilities", async ({
			...
			const output = (
				await runPythonScript(
					sidecarContainer,
					"present_extension_queryversion_and_querycapabilities.py",
					{ env: { DISPLAY: ":99" } },
				)
			).output.trim();
		...
		test("XTEST extension: GetVersion and CompareCursor", async ({
			...
			const output = (
				await runPythonScript(
					sidecarContainer,
					"xtest_extension_getversion_and_comparecursor.py",
					{ env: { DISPLAY: ":99" } },
				)
			).output.trim();
		...
		test("ReparentWindow generates proper events", async ({
			...
			const output = (
				await runPythonScript(
					sidecarContainer,
					"reparentwindow_generates_proper_events.py",
					{ env: { DISPLAY: ":99" } },
				)
			).output.trim();
		...
		test("ReparentWindow rejects circular parent (self-reparent)", async ({
			...
			const output = (
				await runPythonScript(
					sidecarContainer,
					"reparentwindow_rejects_circular_parent_self_reparent.py",
					{ env: { DISPLAY: ":99" } },
				)
			).output.trim();
		...
		test("ReparentWindow rejects reparenting to own descendant", async ({
			...
			const output = (
				await runPythonScript(
					sidecarContainer,
					"reparentwindow_rejects_reparenting_to_own_descendant.py",
					{ env: { DISPLAY: ":99" } },
				)
			).output.trim();
			expect(output).toContain("result=BAD_MATCH");
		...
		test("ReparentWindow generates MapNotify when remapping", async ({
			...
			const output = (
				await runPythonScript(
					sidecarContainer,
					"reparentwindow_generates_mapnotify_when_remapping.py",
					{ env: { DISPLAY: ":99" } },
				)
			).output.trim();
		...
		test("SetDashes rejects zero-length dash values", async ({
			...
			const output = (
				await runPythonScript(
					sidecarContainer,
					"setdashes_rejects_zero_length_dash_values.py",
					{ env: { DISPLAY: ":99" } },
				)
			).output.trim();
		...
		test("SetDashes accepts valid non-zero dash values", async ({
			...
			const output = (
				await runPythonScript(
					sidecarContainer,
					"setdashes_accepts_valid_non_zero_dash_values.py",
					{ env: { DISPLAY: ":99" } },
				)
			).output.trim();
		...
		test("_NET_WM_WINDOW_TYPE property is accepted on windows", async ({
			...
			const output = (
				await runPythonScript(
					sidecarContainer,
					"net_wm_window_type_property_is_accepted_on_windows.py",
					{ env: { DISPLAY: ":99" } },
				)
			).output.trim();
			expect(output).toContain("result=OK");
		...
		test("_NET_WM_STRUT updates _NET_WORKAREA on root", async ({
			...
			const output = (
				await runPythonScript(
					sidecarContainer,
					"net_wm_strut_updates_net_workarea_on_root.py",
					{ env: { DISPLAY: ":99" } },
				)
			).output.trim();
			expect(output).toContain("result=OK");
		...
		test("system tray manager is advertised via _NET_SYSTEM_TRAY_S0", async ({
			...
			const output = (
				await runPythonScript(
					sidecarContainer,
					"system_tray_manager_is_advertised_via_net_system_tray_s0.py",
					{ env: { DISPLAY: ":99" } },
				)
			).output.trim();
			expect(output).toContain("result=OK");
		...
		test("_NET_WM_STATE can toggle ABOVE state", async ({
			...
			const output = (
				await runPythonScript(
					sidecarContainer,
					"net_wm_state_can_toggle_above_state.py",
					{ env: { DISPLAY: ":99" } },
				)
			).output.trim();
			expect(output).toContain("result=OK");
		...
		test("_NET_CLOSE_WINDOW sends WM_DELETE_WINDOW to compliant windows", async ({
			...
			const output = (
				await runPythonScript(
					sidecarContainer,
					"net_close_window_sends_wm_delete_window_to_compliant_windows.py",
					{ env: { DISPLAY: ":99" } },
				)
			).output.trim();
		...
		test("_NET_CLIENT_LIST is maintained on root", async ({
			...
			const output = (
				await runPythonScript(
					sidecarContainer,
					"net_client_list_is_maintained_on_root.py",
					{ env: { DISPLAY: ":99" } },
				)
			).output.trim();
			expect(output).toContain("result=OK");
		...
		test("_NET_ACTIVE_WINDOW tracks focused window", async ({
			...
			const output = (
				await runPythonScript(
					sidecarContainer,
					"net_active_window_tracks_focused_window.py",
					{ env: { DISPLAY: ":99" } },
				)
			).output.trim();
			expect(output).toContain("result=OK");
		...
		test("XSETTINGS manager is advertised and provides settings", async ({
			...
			const output = (
				await runPythonScript(
					sidecarContainer,
					"xsettings_manager_is_advertised_and_provides_settings.py",
					{ env: { DISPLAY: ":99" } },
				)
			).output.trim();
			expect(output).toContain("result=OK");
		...
		test("WM_TRANSIENT_FOR stacking: transient above parent", async ({
			...
			const output = (
				await runPythonScript(
					sidecarContainer,
					"wm_transient_for_stacking_transient_above_parent.py",
					{ env: { DISPLAY: ":99" } },
				)
			).output.trim();
			expect(output).toContain("result=OK");
		...
		test("QueryExtension returns unique major opcodes for all extensions", async ({
			...
			const output = (
				await runPythonScript(
					sidecarContainer,
					"queryextension_returns_unique_major_opcodes_for_all_extensions.py",
					{ env: { DISPLAY: ":99" } },
				)
			).output.trim();
		...
		test("SYNC extension events use correct event base", async ({
			...
			const output = (
				await runPythonScript(
					sidecarContainer,
					"sync_extension_events_use_correct_event_base.py",
					{ env: { DISPLAY: ":99" } },
				)
			).output.trim();
		...
		test("RENDER extension reports first_error for BadPictFormat", async ({
			...
			const output = (
				await runPythonScript(
					sidecarContainer,
					"render_extension_reports_first_error_for_badpictformat.py",
					{ env: { DISPLAY: ":99" } },
				)
			).output.trim();
			expect(output).toContain("present=True");
		...
		test("Extension event bases do not overlap", async ({
			...
			const output = (
				await runPythonScript(
					sidecarContainer,
					"extension_event_bases_do_not_overlap.py",
					{ env: { DISPLAY: ":99" } },
				)
			).output.trim();
			expect(output).toContain("OK:");
...
test.describe
	.serial("RENDER extension compliance", () => {
		test.setTimeout(120_000);
		...
		test("rendercheck gradient operations", async ({ sidecarContainer }) => {
			const output = await execInSidecar(
				sidecarContainer,
		...
		test("rendercheck blend operations", async ({ sidecarContainer }) => {
			const output = await execInSidecar(
				sidecarContainer,
		...
		test("rendercheck composite operations", async ({ sidecarContainer }) => {
			const output = await execInSidecar(
				sidecarContainer,
		...
		test("RENDER PictFormats include required formats", async ({
			...
			const output = (
				await runPythonScript(
					sidecarContainer,
					"render_pictformats_include_required_formats.py",
					{ env: { DISPLAY: ":99" } },
				)
			).output.trim();
...
test.describe
	.serial("Application smoke tests", () => {
		test.setTimeout(120_000);
		...
		test("xprop lists root window properties", async ({ sidecarContainer }) => {
			const output = await execInSidecar(
				sidecarContainer,
