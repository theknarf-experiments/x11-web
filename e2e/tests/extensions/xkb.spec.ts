...
test.describe.serial("Application compatibility", () => {
	...
	test("server reports all required extensions", async ({
		...
		const output = (await runPythonScript(sidecarContainer, "server_reports_all_required_extensions.py", { env: { DISPLAY: ":99" } })).output.trim();
	...
	test("server reports correct screen info", async ({
		...
		const output = (await runPythonScript(sidecarContainer, "server_reports_correct_screen_info.py", { env: { DISPLAY: ":99" } })).output.trim();
	...
	test("standard and EWMH atoms are present", async ({
		...
		const output = (await runPythonScript(sidecarContainer, "standard_and_ewmh_atoms_are_present.py", { env: { DISPLAY: ":99" } })).output.trim();
	...
	test("RANDR provides screen information", async ({ sidecarContainer }) => {
		const output = (await runPythonScript(sidecarContainer, "randr_provides_screen_information.py", { env: { DISPLAY: ":99" } })).output.trim();
	...
	test("Font system serves standard fonts", async ({ sidecarContainer }) => {
		const output = (await runPythonScript(sidecarContainer, "font_system_serves_standard_fonts.py", { env: { DISPLAY: ":99" } })).output.trim();
	...
	test("Visual configuration supports TrueColor", async ({
		...
		const output = (await runPythonScript(sidecarContainer, "visual_configuration_supports_truecolor.py", { env: { DISPLAY: ":99" } })).output.trim();
	...
	test("XSETTINGS manager provides GTK defaults", async ({
		...
		const output = (await runPythonScript(sidecarContainer, "xsettings_manager_provides_gtk_defaults.py", { env: { DISPLAY: ":99" } })).output.trim();
	...
	test("XKB extension is available", async ({ sidecarContainer }) => {
		const output = (await runPythonScript(sidecarContainer, "xkb_extension_is_available.py", { env: { DISPLAY: ":99" } })).output.trim();
	...
	test("X11 window create/map/destroy cycle works", async ({
		...
		const output = (await runPythonScript(sidecarContainer, "x11_window_create_map_destroy_cycle_works.py", { env: { DISPLAY: ":99" } })).output.trim();
	...
	test("_NET_WM_STATE_FULLSCREEN resizes window to screen size", async ({
		...
		const output = (await runPythonScript(sidecarContainer, "net_wm_state_fullscreen_resizes_window_to_screen_size.py", { env: { DISPLAY: ":99" } })).output.trim();
	...
	test("_NET_WM_STATE_MAXIMIZED resizes window", async ({
		...
		const output = (await runPythonScript(sidecarContainer, "net_wm_state_maximized_resizes_window.py", { env: { DISPLAY: ":99" } })).output.trim();
...
echo "xlogo=$(pgrep xlogo | wc -l)"`,
	...
	test("Window stacking z-order is maintained", async ({
		...
		const output = (await runPythonScript(sidecarContainer, "window_stacking_z_order_is_maintained.py", { env: { DISPLAY: ":99" } })).output.trim();
	...
	test("_NET_SUPPORTED has 20+ atoms", async ({ sidecarContainer }) => {
		const output = (await runPythonScript(sidecarContainer, "net_supported_has_20_atoms.py", { env: { DISPLAY: ":99" } })).output.trim();
		const cnt = Number.parseInt(
	...
	test("window configure and query works", async ({ sidecarContainer }) => {
		const output = (await runPythonScript(sidecarContainer, "window_configure_and_query_works.py", { env: { DISPLAY: ":99" } })).output.trim();
	...
	test("All required extensions are present", async ({
		...
		const output = (await runPythonScript(sidecarContainer, "all_required_extensions_are_present.py", { env: { DISPLAY: ":99" } })).output.trim();
		for (const ext of [
			"MIT-SHM",
			"RENDER",
			"Composite",
...
test.describe.serial("XKB control masks (Phase 7A)", () => {
	test("SetControls/GetControls round-trips RepeatKeys (bit 0)", async ({
		...
		const output = (await runPythonScript(sidecarContainer, "setcontrols_getcontrols_round_trips_repeatkeys_bit_0.py", { env: { DISPLAY: ":99" } })).output.trim();
	...
	test("XKB extension is queryable", async ({ sidecarContainer }) => {
		const output = (await runPythonScript(sidecarContainer, "xkb_extension_is_queryable.py", { env: { DISPLAY: ":99" } })).output.trim();
		expect(output).toContain("present=True");
	});
});
