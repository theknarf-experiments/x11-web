...
test.describe
	.serial("COMPOSITE extension compliance", () => {
		...
		test("Composite redirect and unredirect window", async ({
			...
			const output = (
				await runPythonScript(
					sidecarContainer,
					"composite_redirect_and_unredirect_window.py",
					{ env: { DISPLAY: ":99" } },
				)
			).output.trim();
		...
		test("Overlay window via Composite GetOverlayWindow", async ({
			...
			const output = (
				await runPythonScript(
					sidecarContainer,
					"overlay_window_via_composite_getoverlaywindow.py",
					{ env: { DISPLAY: ":99" } },
				)
			).output.trim();
...
test.describe("Composite overlay window refcounting", () => {
	test("Composite extension QueryVersion and overlay operations", async ({
		...
		const result = await runPythonScript(
			sidecarContainer,
			"composite_overlay_get_release.py",
			{ env: { DISPLAY: ":99" } },
		);
...
test.describe
	.serial("COMPOSITE extension (Phase 7)", () => {
		test("CompositeQueryVersion returns 0.4+", async ({ sidecarContainer }) => {
			const output = (
				await runPythonScript(
					sidecarContainer,
					"compositequeryversion_returns_0_4.py",
					{ env: { DISPLAY: ":99" } },
				)
			).output.trim();
			expect(output).toContain("composite_present=True");
		...
		test("NameWindowPixmap creates valid pixmap", async ({
			...
			const output = (
				await runPythonScript(
					sidecarContainer,
					"namewindowpixmap_creates_valid_pixmap.py",
					{ env: { DISPLAY: ":99" } },
				)
			).output.trim();
