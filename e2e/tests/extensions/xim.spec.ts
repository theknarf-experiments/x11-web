...
test.describe
	.serial("XIM protocol compliance", () => {
		test("XIM server window exists on display", async ({
			...
			const output = (
				await runPythonScript(
					sidecarContainer,
					"xim_server_window_exists_on_display.py",
					{ env: { DISPLAY: ":99" } },
				)
			).output.trim();
		...
		test("xterm launches without XIM errors", async ({ sidecarContainer }) => {
