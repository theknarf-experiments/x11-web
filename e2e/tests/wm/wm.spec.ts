...
test.describe
	.serial("Window manager compliance", () => {
		...
		test("override-redirect windows bypass SubstructureRedirect", async ({
			...
			const output = (
				await runPythonScript(
					sidecarContainer,
					"override_redirect_windows_bypass_substructureredirect.py",
					{ env: { DISPLAY: ":99" } },
				)
			).output.trim();
		...
		test("window stacking operations (raise/lower)", async ({
			...
			const output = (
				await runPythonScript(
					sidecarContainer,
					"window_stacking_operations_raise_lower.py",
					{ env: { DISPLAY: ":99" } },
				)
			).output.trim();
		...
		test("focus model (Passive, Locally Active) works", async ({
			...
			const output = (
				await runPythonScript(
					sidecarContainer,
					"focus_model_passive_locally_active_works.py",
					{ env: { DISPLAY: ":99" } },
				)
			).output.trim();
