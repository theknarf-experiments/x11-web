...
import { expect, test, runPythonScript } from "./fixtures";
...
test.describe.serial("WM_STATE ICCCM compliance", () => {
	...
	test("WM_STATE is set when window is mapped", async ({
		...
		const output = (await runPythonScript(sidecarContainer, "wm_state_is_set_when_window_is_mapped.py", { env: { DISPLAY: ":99" } })).output.trim();
	...
	test("WM_STATE is NormalState for child windows", async ({
		...
		const output = (await runPythonScript(sidecarContainer, "wm_state_is_normalstate_for_child_windows.py", { env: { DISPLAY: ":99" } })).output.trim();
...
test.describe.serial("Window management edge cases", () => {
	...
	test("Expose event on newly mapped window with ExposureMask", async ({
		...
		const output = (await runPythonScript(sidecarContainer, "expose_event_on_newly_mapped_window_with_exposuremask.py", { env: { DISPLAY: ":99" } })).output.trim();
	...
	test("UnmapNotify sent when window unmapped", async ({
		...
		const output = (await runPythonScript(sidecarContainer, "unmapnotify_sent_when_window_unmapped.py", { env: { DISPLAY: ":99" } })).output.trim();
