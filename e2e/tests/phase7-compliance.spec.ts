...
test.describe.serial("ConfigureNotify delivery (Phase 7)", () => {
	test("Window receives ConfigureNotify on resize when StructureNotifyMask set", async ({
		...
		const output = (await runPythonScript(sidecarContainer, "window_receives_configurenotify_on_resize_when_structurenotifymask_set.py", { env: { DISPLAY: ":99" } })).output.trim();
	...
	test("MapNotify delivered only with StructureNotifyMask", async ({
		...
		const output = (await runPythonScript(sidecarContainer, "mapnotify_delivered_only_with_structurenotifymask.py", { env: { DISPLAY: ":99" } })).output.trim();
...
test.describe.serial("Drawable depth handling (Phase 7)", () => {
	test("CreatePixmap with depth 1 works correctly", async ({
		...
		const output = (await runPythonScript(sidecarContainer, "createpixmap_with_depth_1_works_correctly.py", { env: { DISPLAY: ":99" } })).output.trim();
	...
	test("Window depth matches screen root depth", async ({
		...
		const output = (await runPythonScript(sidecarContainer, "window_depth_matches_screen_root_depth.py", { env: { DISPLAY: ":99" } })).output.trim();
...
test.describe.serial("Selection protocol (Phase 7)", () => {
	test("SetSelectionOwner and GetSelectionOwner round-trip", async ({
		...
		const output = (await runPythonScript(sidecarContainer, "setselectionowner_and_getselectionowner_round_trip.py", { env: { DISPLAY: ":99" } })).output.trim();
	...
	test("TARGETS selection target is supported", async ({
		...
		const output = (await runPythonScript(sidecarContainer, "targets_selection_target_is_supported.py", { env: { DISPLAY: ":99" } })).output.trim();
...
test.describe.serial("EWMH compliance (Phase 7)", () => {
	test("_NET_SUPPORTED includes critical atoms", async ({
		...
		const output = (await runPythonScript(sidecarContainer, "net_supported_includes_critical_atoms.py", { env: { DISPLAY: ":99" } })).output.trim();
	...
	test("WM_DELETE_WINDOW protocol works", async ({ sidecarContainer }) => {
		const output = (await runPythonScript(sidecarContainer, "wm_delete_window_protocol_works.py", { env: { DISPLAY: ":99" } })).output.trim();
