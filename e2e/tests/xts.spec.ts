...
import { test, expect, spawnApp, waitForDock, hasRenderedContent, canvasPixelHash, waitForCanvasStable, runPythonScript } from "./fixtures";
...
test.describe.serial("X11 protocol compliance", () => {
	...
	test("InternAtom and GetAtomName round-trip", async ({
		...
		const output = (await runPythonScript(sidecarContainer, "internatom_and_getatomname_round_trip_3.py", { env: { DISPLAY: ":99" } })).output.trim();
	...
	test("CreateWindow and MapWindow work correctly", async ({
		...
		const output = (await runPythonScript(sidecarContainer, "createwindow_and_mapwindow_work_correctly.py", { env: { DISPLAY: ":99" } })).output.trim();
	...
	test("GetWindowAttributes returns correct your_event_mask", async ({
		...
		const output = (await runPythonScript(sidecarContainer, "getwindowattributes_returns_correct_your_event_mask.py", { env: { DISPLAY: ":99" } })).output.trim();
	...
	test("ChangeProperty and GetProperty round-trip", async ({
		...
		const output = (await runPythonScript(sidecarContainer, "changeproperty_and_getproperty_round_trip_2.py", { env: { DISPLAY: ":99" } })).output.trim();
	...
	test("QueryTree returns correct window hierarchy", async ({
		...
		const output = (await runPythonScript(sidecarContainer, "querytree_returns_correct_window_hierarchy.py", { env: { DISPLAY: ":99" } })).output.trim();
	...
	test("GrabPointer and UngrabPointer work", async ({
		...
		const output = (await runPythonScript(sidecarContainer, "grabpointer_and_ungrabpointer_work.py", { env: { DISPLAY: ":99" } })).output.trim();
	...
	test("SelectionOwner set/get round-trip", async ({
		...
		const output = (await runPythonScript(sidecarContainer, "selectionowner_set_get_round_trip.py", { env: { DISPLAY: ":99" } })).output.trim();
		expect(output).toContain("owner_matches=True");
	});

	test("Colormap operations work", async ({ sidecarContainer }) => {
		const output = (await runPythonScript(sidecarContainer, "colormap_operations_work.py", { env: { DISPLAY: ":99" } })).output.trim();
	...
	test("RENDER extension QueryVersion succeeds", async ({
		...
		const output = (await runPythonScript(sidecarContainer, "render_extension_queryversion_succeeds.py", { env: { DISPLAY: ":99" } })).output.trim();
	...
	test("QueryPointer returns valid coordinates", async ({
		...
		const output = (await runPythonScript(sidecarContainer, "querypointer_returns_valid_coordinates_2.py", { env: { DISPLAY: ":99" } })).output.trim();
	...
	test("TranslateCoordinates works correctly", async ({
		...
		const output = (await runPythonScript(sidecarContainer, "translatecoordinates_works_correctly.py", { env: { DISPLAY: ":99" } })).output.trim();
	...
	test("ConfigureWindow changes geometry", async ({
		...
		const output = (await runPythonScript(sidecarContainer, "configurewindow_changes_geometry.py", { env: { DISPLAY: ":99" } })).output.trim();
	...
	test("ListExtensions returns comprehensive list", async ({
		...
		const output = (await runPythonScript(sidecarContainer, "listextensions_returns_comprehensive_list.py", { env: { DISPLAY: ":99" } })).output.trim();
	...
	test("CreateGC and drawing operations work", async ({
		...
		const output = (await runPythonScript(sidecarContainer, "creategc_and_drawing_operations_work.py", { env: { DISPLAY: ":99" } })).output.trim();
	...
	test("multiple simultaneous X11 clients work", async ({
		...
		const output = (await runPythonScript(sidecarContainer, "multiple_simultaneous_x11_clients_work.py", { env: { DISPLAY: ":99" } })).output.trim();
	...
	test("stress: rapid window create/destroy cycle", async ({
		...
		test.setTimeout(30_000);
		const output = (await runPythonScript(sidecarContainer, "stress_rapid_window_create_destroy_cycle.py", { env: { DISPLAY: ":99" } })).output.trim();
	...
	test("stress: rapid property set/get cycle", async ({
		...
		const output = (await runPythonScript(sidecarContainer, "stress_rapid_property_set_get_cycle.py", { env: { DISPLAY: ":99" } })).output.trim();
	...
	test("BAD_LENGTH on truncated requests", async ({
		...
		const output = (await runPythonScript(sidecarContainer, "bad_length_on_truncated_requests.py", { env: { DISPLAY: ":99" } })).output.trim();
