...
test.describe
	.serial("Resource limits and robustness", () => {
		test("server handles rapid window create/destroy without leaking", async ({
			...
			const output = (
				await runPythonScript(
					sidecarContainer,
					"server_handles_rapid_window_create_destroy_without_leaking.py",
					{ env: { DISPLAY: ":99" } },
				)
			).output.trim();
		...
		test("server handles rapid pixmap create/free without leaking", async ({
			...
			const output = (
				await runPythonScript(
					sidecarContainer,
					"server_handles_rapid_pixmap_create_free_without_leaking.py",
					{ env: { DISPLAY: ":99" } },
				)
			).output.trim();
			expect(output).toContain("created=500");
		...
		test("server handles rapid GC create/free without leaking", async ({
			...
			const output = (
				await runPythonScript(
					sidecarContainer,
					"server_handles_rapid_gc_create_free_without_leaking.py",
					{ env: { DISPLAY: ":99" } },
				)
			).output.trim();
			expect(output).toContain("created=500");
		...
		test("server stays responsive under event flood", async ({
			...
			const output = (
				await runPythonScript(
					sidecarContainer,
					"server_stays_responsive_under_event_flood.py",
					{ env: { DISPLAY: ":99" } },
				)
			).output.trim();
		...
		test("server survives many sequential connections", async ({
			...
			const output = (
				await runPythonScript(
					sidecarContainer,
					"server_survives_many_sequential_connections.py",
					{ env: { DISPLAY: ":99" } },
				)
			).output.trim();
...
test.describe("Multi-app interaction", () => {
	...
	test("20 rapid window create/destroy cycles don't crash", async ({
		...
		const result = await runPythonScript(
			sidecarContainer,
			"rapid_window_create_destroy_20_cycles.py",
			{ env: { DISPLAY: ":99" } },
		);
	...
	.serial("Protocol edge cases", () => {
		...
		test("Large property data (INCR threshold)", async ({
			...
			const output = (
				await runPythonScript(
					sidecarContainer,
					"large_property_data_incr_threshold.py",
					{ env: { DISPLAY: ":99" } },
				)
			).output.trim();
		...
		test("Window hierarchy: reparent, query tree", async ({
			...
			const output = (
				await runPythonScript(
					sidecarContainer,
					"window_hierarchy_reparent_query_tree.py",
					{ env: { DISPLAY: ":99" } },
				)
			).output.trim();
		...
		test("Colormap operations", async ({ sidecarContainer }) => {
			const output = (
				await runPythonScript(sidecarContainer, "colormap_operations.py", {
					env: { DISPLAY: ":99" },
				})
			).output.trim();
		...
		test("GrabPointer and UngrabPointer", async ({ sidecarContainer }) => {
			const output = (
				await runPythonScript(
					sidecarContainer,
					"grabpointer_and_ungrabpointer.py",
					{ env: { DISPLAY: ":99" } },
				)
			).output.trim();
		...
		test("SetInputFocus and FocusIn/FocusOut events", async ({
			...
			const output = (
				await runPythonScript(
					sidecarContainer,
					"setinputfocus_and_focusin_focusout_events.py",
					{ env: { DISPLAY: ":99" } },
				)
			).output.trim();
		...
		test("CreatePixmap at multiple depths", async ({ sidecarContainer }) => {
			const output = (
				await runPythonScript(
					sidecarContainer,
					"createpixmap_at_multiple_depths.py",
					{ env: { DISPLAY: ":99" } },
				)
			).output.trim();
		...
		test("Window stacking order operations", async ({ sidecarContainer }) => {
			const output = (
				await runPythonScript(
					sidecarContainer,
					"window_stacking_order_operations.py",
					{ env: { DISPLAY: ":99" } },
				)
			).output.trim();
		...
		test("RENDER CreatePicture and Composite", async ({ sidecarContainer }) => {
			const output = (
				await runPythonScript(
					sidecarContainer,
					"render_createpicture_and_composite.py",
					{ env: { DISPLAY: ":99" } },
				)
			).output.trim();
		...
		test("SYNC extension counter operations", async ({ sidecarContainer }) => {
			const output = (
				await runPythonScript(
					sidecarContainer,
					"sync_extension_counter_operations.py",
					{ env: { DISPLAY: ":99" } },
				)
			).output.trim();
		...
		test("Rapid connect/disconnect stress test", async ({
			...
			const output = (
				await runPythonScript(
					sidecarContainer,
					"rapid_connect_disconnect_stress_test.py",
					{ env: { DISPLAY: ":99" } },
				)
			).output.trim();
...
test.describe
	.serial("Multi-client concurrent stress tests", () => {
		test("rapid window create/destroy cycle (100 windows)", async ({
			...
			const output = (
				await runPythonScript(
					sidecarContainer,
					"rapid_window_create_destroy_cycle_100_windows.py",
					{ env: { DISPLAY: ":99" } },
				)
			).output.trim();
		...
		test("concurrent connections (10 simultaneous clients)", async ({
			...
			const output = (
				await runPythonScript(
					sidecarContainer,
					"concurrent_connections_10_simultaneous_clients.py",
					{ env: { DISPLAY: ":99" } },
				)
			).output.trim();
		...
		test("property change storm (1000 rapid property changes)", async ({
			...
			const output = (
				await runPythonScript(
					sidecarContainer,
					"property_change_storm_1000_rapid_property_changes.py",
					{ env: { DISPLAY: ":99" } },
				)
			).output.trim();
		...
		test("atom interning stress (500 unique atoms)", async ({
			...
			const output = (
				await runPythonScript(
					sidecarContainer,
					"atom_interning_stress_500_unique_atoms.py",
					{ env: { DISPLAY: ":99" } },
				)
			).output.trim();
			expect(output).toContain("result=OK");
		...
		test("rapid grab/ungrab cycles", async ({ sidecarContainer }) => {
			...
			const output = (
				await runPythonScript(sidecarContainer, "rapid_grab_ungrab_cycles.py", {
					env: { DISPLAY: ":99" },
				})
			).output.trim();
			expect(output).toContain("result=OK");
		...
		test("deep window hierarchy (50 levels of nesting)", async ({
			...
			const output = (
				await runPythonScript(
					sidecarContainer,
					"deep_window_hierarchy_50_levels_of_nesting.py",
					{ env: { DISPLAY: ":99" } },
				)
			).output.trim();
			expect(output).toContain("result=OK");
...
test.describe("Multi-client stress", () => {
	test("10 concurrent X11 connections with window operations", async ({
		...
		const result = await runPythonScript(
			sidecarContainer,
			"concurrent_x11_window_operations.py",
			{ env: { DISPLAY: ":99" } },
		);
...
test.describe("Concurrent client stress tests", () => {
	...
	test("rapid connect/disconnect cycles", async ({ sidecarContainer }) => {
		...
		const result = await runPythonScript(
			sidecarContainer,
			"rapid_connect_disconnect_100_cycles.py",
			{ env: { DISPLAY: ":99" } },
		);
...
test.describe
	.serial("Multi-client stress tests", () => {
		test("100 rapid window create/destroy cycles", async ({
			...
			const output = (
				await runPythonScript(
					sidecarContainer,
					"100_rapid_window_create_destroy_cycles.py",
					{ env: { DISPLAY: ":99" } },
				)
			).output.trim();
		...
		test("500 unique atoms can be interned", async ({ sidecarContainer }) => {
			const output = (
				await runPythonScript(
					sidecarContainer,
					"500_unique_atoms_can_be_interned.py",
					{ env: { DISPLAY: ":99" } },
				)
			).output.trim();
		...
		test("1000 rapid property changes on single window", async ({
			...
			const output = (
				await runPythonScript(
					sidecarContainer,
					"1000_rapid_property_changes_on_single_window.py",
					{ env: { DISPLAY: ":99" } },
				)
			).output.trim();
...
test.describe("Protocol fuzzing", () => {
	test("server survives truncated requests", async ({ sidecarContainer }) => {
		...
		const result = await runPythonScript(
			sidecarContainer,
			"server_survives_truncated_requests.py",
			{ env: { DISPLAY: ":99" } },
		);
	...
	test("server handles zero-size drawables gracefully", async ({
		...
		const result = await runPythonScript(
			sidecarContainer,
			"server_handles_zero_size_drawables.py",
			{ env: { DISPLAY: ":99" } },
		);
...
test.describe("Conformance: Protocol fuzzing", () => {
	test("server survives malformed requests", async ({ sidecarContainer }) => {
		...
		const result = await runPythonScript(
			sidecarContainer,
			"server_survives_malformed_requests.py",
			{ env: { DISPLAY: ":99" } },
		);
		...
		const alive = await sidecarContainer
			.exec(["true"])
			.then(() => true)
			.catch(() => false);
	...
	test("server handles rapid connect-disconnect cycles", async ({
		...
		const result = await runPythonScript(
			sidecarContainer,
			"rapid_connect_disconnect_cycles.py",
			{ env: { DISPLAY: ":99" } },
		);
...
test.describe("Conformance: Stress and edge cases", () => {
	test("rapid window create/destroy cycle", async ({ sidecarContainer }) => {
		const result = await runPythonScript(
			sidecarContainer,
			"rapid_window_create_destroy.py",
			{ env: { DISPLAY: ":99" } },
		);
	...
	test("large property data round-trip", async ({ sidecarContainer }) => {
		const result = await runPythonScript(
			sidecarContainer,
			"large_property_data_roundtrip.py",
			{ env: { DISPLAY: ":99" } },
		);
	...
	test("multiple simultaneous connections", async ({ sidecarContainer }) => {
		const result = await runPythonScript(
			sidecarContainer,
			"multiple_simultaneous_connections.py",
			{ env: { DISPLAY: ":99" } },
		);
	...
	test("deeply nested window hierarchy", async ({ sidecarContainer }) => {
		const result = await runPythonScript(
			sidecarContainer,
			"deeply_nested_window_hierarchy.py",
			{ env: { DISPLAY: ":99" } },
		);
...
test.describe("BadLength error handling", () => {
	test("server returns BadLength for truncated CreateWindow", async ({
		...
		const result = await runPythonScript(
			sidecarContainer,
			"badlength_truncated_createwindow.py",
			{ env: { DISPLAY: ":99" } },
		);
...
test.describe("Extended protocol conformance", () => {
	...
	test("concurrent connections operate independently", async ({
		...
		const result = await runPythonScript(
			sidecarContainer,
			"concurrent_connections_independent.py",
			{ env: { DISPLAY: ":99" } },
		);
	...
	test("window reparenting and QueryTree correctness", async ({
		...
		const result = await runPythonScript(
			sidecarContainer,
			"window_reparenting_querytree.py",
			{ env: { DISPLAY: ":99" } },
		);
		expect(result.output).toContain(
			"PASS: child geometry correct after reparent",
		);
	});
	...
	test("event mask filtering delivers correct events", async ({
		...
		const result = await runPythonScript(
			sidecarContainer,
			"event_mask_filtering_propnotify.py",
			{ env: { DISPLAY: ":99" } },
		);
		expect(result.output).toContain("PASS");
	});
	...
	test("GrabPointer and UngrabPointer", async ({ sidecarContainer }) => {
		...
		const result = await runPythonScript(
			sidecarContainer,
			"grabpointer_ungrabpointer_extended.py",
			{ env: { DISPLAY: ":99" } },
		);
	...
	test("xrestop can query resource usage", async ({ sidecarContainer }) => {
		...
		const result = await runPythonScript(
			sidecarContainer,
			"xrestop_query_resource_usage.py",
			{ env: { DISPLAY: ":99" } },
		);
		expect(result.output).toContain("PASS");
	});
	...
	test("SHAPE extension creates non-rectangular windows", async ({
		...
		const result = await runPythonScript(
			sidecarContainer,
			"shape_extension_nonrect_windows.py",
			{ env: { DISPLAY: ":99" } },
		);
		expect(result.output).toContain("PASS");
	});
	...
	test("RECORD extension is available", async ({ sidecarContainer }) => {
		const result = await runPythonScript(
			sidecarContainer,
			"record_extension_available_simple.py",
			{ env: { DISPLAY: ":99" } },
		);
		expect(result.output).toContain("PASS");
	});
	...
	test("SECURITY extension is available", async ({ sidecarContainer }) => {
		const result = await runPythonScript(
			sidecarContainer,
			"security_extension_available.py",
			{ env: { DISPLAY: ":99" } },
		);
		expect(result.output).toContain("PASS");
	});
	...
	test("SDL2 can open a display connection", async ({ sidecarContainer }) => {
		const result = await runPythonScript(
			sidecarContainer,
			"sdl2_open_display_connection.py",
			{ env: { DISPLAY: ":99" } },
		);
		expect(result.output).toContain("PASS");
	});
	...
	test("multiple rapid connect/disconnect cycles don't leak", async ({
		...
		const result = await runPythonScript(
			sidecarContainer,
			"rapid_connect_disconnect_no_leak.py",
			{ env: { DISPLAY: ":99" } },
		);
	...
	test("InputOnly windows can receive events", async ({ sidecarContainer }) => {
		...
		const result = await runPythonScript(
			sidecarContainer,
			"inputonly_window_receives_events.py",
			{ env: { DISPLAY: ":99" } },
		);
		expect(result.output).toContain("PASS");
	});
	...
	test("GetImage returns pixel data from drawn window", async ({
		...
		const result = await runPythonScript(
			sidecarContainer,
			"getimage_pixel_data_from_drawn_window.py",
			{ env: { DISPLAY: ":99" } },
		);
		expect(result.output).toContain("PASS");
	});
