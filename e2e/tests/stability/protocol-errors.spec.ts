...
test.describe
	.serial("Real application deep tests", () => {
		...
		test("xeyes starts, runs, and exits cleanly", async ({
			...
			await execInSidecar(
				sidecarContainer,
				"pkill -9 xeyes 2>/dev/null; sleep 2",
			);
		...
		test("xterm renders with multiple fonts", async ({ sidecarContainer }) => {
...
echo EXIT_CODE=$?`,
		...
		test("Xlib finds root visual in setup (no NULL visual pointer)", async ({
			...
			await sidecarContainer.exec([
				"bash",
				"-c",
				`printf '%s' '${b64}' | base64 -d > /tmp/visual_test.py`,
			]);
			const output = await execInSidecar(
				sidecarContainer,
				"python3 /tmp/visual_test.py 2>&1",
			);
...
test.describe
	.serial("Edge case protocol compliance", () => {
		test("zero-size window creation is rejected (BadValue)", async ({
			...
			const output = (
				await runPythonScript(
					sidecarContainer,
					"zero_size_window_creation_is_rejected_badvalue.py",
					{ env: { DISPLAY: ":99" } },
				)
			).output.trim();
		...
		test("GetGeometry on root window returns screen dimensions", async ({
			...
			const output = (
				await runPythonScript(
					sidecarContainer,
					"getgeometry_on_root_window_returns_screen_dimensions.py",
					{ env: { DISPLAY: ":99" } },
				)
			).output.trim();
		...
		test("InternAtom only_if_exists=True returns 0 for unknown atoms", async ({
			...
			const output = (
				await runPythonScript(
					sidecarContainer,
					"internatom_only_if_exists_true_returns_0_for_unknown_atoms.py",
					{ env: { DISPLAY: ":99" } },
				)
			).output.trim();
		...
		test("GetProperty with delete=True removes property", async ({
			...
			const output = (
				await runPythonScript(
					sidecarContainer,
					"getproperty_with_delete_true_removes_property.py",
					{ env: { DISPLAY: ":99" } },
				)
			).output.trim();
		...
		test("SendEvent delivers synthetic events with send_event flag", async ({
			...
			const output = (
				await runPythonScript(
					sidecarContainer,
					"sendevent_delivers_synthetic_events_with_send_event_flag.py",
					{ env: { DISPLAY: ":99" } },
				)
			).output.trim();
		...
		test("CopyArea between pixmap and window", async ({ sidecarContainer }) => {
			const output = (
				await runPythonScript(
					sidecarContainer,
					"copyarea_between_pixmap_and_window.py",
					{ env: { DISPLAY: ":99" } },
				)
			).output.trim();
		...
		test("GC tile and stipple fill modes", async ({ sidecarContainer }) => {
			const output = (
				await runPythonScript(
					sidecarContainer,
					"gc_tile_and_stipple_fill_modes.py",
					{ env: { DISPLAY: ":99" } },
				)
			).output.trim();
		...
		test("KeyPress/KeyRelease event delivery via XTEST", async ({
			...
			const output = (
				await runPythonScript(
					sidecarContainer,
					"keypress_keyrelease_event_delivery_via_xtest.py",
					{ env: { DISPLAY: ":99" } },
				)
			).output.trim();
		...
		test("ConfigureNotify includes correct fields per spec", async ({
			...
			const output = (
				await runPythonScript(
					sidecarContainer,
					"configurenotify_includes_correct_fields_per_spec.py",
					{ env: { DISPLAY: ":99" } },
				)
			).output.trim();
		...
		test("MapNotify and UnmapNotify event sequence", async ({
			...
			const output = (
				await runPythonScript(
					sidecarContainer,
					"mapnotify_and_unmapnotify_event_sequence.py",
					{ env: { DISPLAY: ":99" } },
				)
			).output.trim();
		...
		test("InputOnly window rejects drawing operations", async ({
			...
			const output = (
				await runPythonScript(
					sidecarContainer,
					"inputonly_window_rejects_drawing_operations.py",
					{ env: { DISPLAY: ":99" } },
				)
			).output.trim();
		...
		test("Override-redirect window bypasses WM intervention", async ({
			...
			const output = (
				await runPythonScript(
					sidecarContainer,
					"override_redirect_window_bypasses_wm_intervention.py",
					{ env: { DISPLAY: ":99" } },
				)
			).output.trim();
		...
		test("INCR selection transfer for large data", async ({
			...
			const output = (
				await runPythonScript(
					sidecarContainer,
					"incr_selection_transfer_for_large_data.py",
					{ env: { DISPLAY: ":99" } },
				)
			).output.trim();
		...
		test("FocusIn/FocusOut events with correct detail codes", async ({
			...
			const output = (
				await runPythonScript(
					sidecarContainer,
					"focusin_focusout_events_with_correct_detail_codes.py",
					{ env: { DISPLAY: ":99" } },
				)
			).output.trim();
		...
		test("Colormap installation and notification", async ({
			...
			const output = (
				await runPythonScript(
					sidecarContainer,
					"colormap_installation_and_notification.py",
					{ env: { DISPLAY: ":99" } },
				)
			).output.trim();
		...
		test("QueryColors returns correct RGB values", async ({
			...
			const output = (
				await runPythonScript(
					sidecarContainer,
					"querycolors_returns_correct_rgb_values.py",
					{ env: { DISPLAY: ":99" } },
				)
			).output.trim();
			expect(output).toContain("is_red=True");
		...
		test("LookupColor returns named color values", async ({
			...
			const output = (
				await runPythonScript(
					sidecarContainer,
					"lookupcolor_returns_named_color_values.py",
					{ env: { DISPLAY: ":99" } },
				)
			).output.trim();
			expect(output).toContain("is_red=True");
		...
		test("xclip round-trip clipboard test", async ({ sidecarContainer }) => {
			...
			if (output.includes("clipboard_test_data")) {
				...
				console.log(
					"xclip clipboard round-trip: partial (expected in container)",
				);
			}
		});
		...
		test("xdotool window management operations", async ({
			...
			await execInSidecar(
				sidecarContainer,
				"xterm -T 'xdotool_test' -e 'sleep 30' &",
			);
			...
			if (wid) {
				...
				await execInSidecar(
					sidecarContainer,
					`xdotool windowmove ${wid} 200 200`,
				);
				// Resize the window
				await execInSidecar(
					sidecarContainer,
					`xdotool windowsize ${wid} 400 300`,
				);
				// Focus the window
			...
			await execInSidecar(
				sidecarContainer,
				"pkill -f 'xdotool_test' 2>/dev/null; true",
			);
...
test.describe("Protocol robustness", () => {
	test("server survives malformed requests without crashing", async ({
		...
		const result = await runPythonScript(
			sidecarContainer,
			"server_survives_malformed_requests_robustness.py",
			{ env: { DISPLAY: ":99" } },
		);
...
test.describe("Protocol compliance: error handling", () => {
	test("server returns proper errors for invalid requests", async ({
		...
		const result = await runPythonScript(
			sidecarContainer,
			"server_returns_proper_errors_invalid_requests.py",
			{ env: { DISPLAY: ":99" } },
		);
...
test.describe("X11 error code verification", () => {
	...
	test("BadValue error on CreatePixmap with zero dimensions", async ({
		...
		const result = await runPythonScript(
			sidecarContainer,
			"badvalue_createpixmap.py",
			{ env: { DISPLAY: ":99" } },
		);
		const match = result.output.match(/errors-badvalue: pass=(\d+) fail=(\d+)/);
		expect(match).toBeTruthy();
		expect(Number.parseInt(match![2], 10)).toBe(0);
	});
	...
	test("BadAtom error on GetAtomName with invalid atom", async ({
		...
		const result = await runPythonScript(
			sidecarContainer,
			"badatom_getatomname.py",
			{ env: { DISPLAY: ":99" } },
		);
		const match = result.output.match(/errors-badatom: pass=(\d+) fail=(\d+)/);
		expect(match).toBeTruthy();
		expect(Number.parseInt(match![2], 10)).toBe(0);
	});
	...
	test("BadColor error on FreeColormap with invalid colormap", async ({
		...
		const result = await runPythonScript(
			sidecarContainer,
			"badcolor_freecolormap.py",
			{ env: { DISPLAY: ":99" } },
		);
		const match = result.output.match(/errors-badcolor: pass=(\d+) fail=(\d+)/);
		expect(match).toBeTruthy();
		expect(Number.parseInt(match![2], 10)).toBe(0);
	});
	...
	test("BadFont error on CloseFont with invalid font", async ({
		...
		const result = await runPythonScript(
			sidecarContainer,
			"badfont_closefont.py",
			{ env: { DISPLAY: ":99" } },
		);
		const match = result.output.match(/errors-badfont: pass=(\d+) fail=(\d+)/);
		expect(match).toBeTruthy();
		expect(Number.parseInt(match![2], 10)).toBe(0);
	});
});
...
test.describe("access control", () => {
	test("xhost lists initial access state", async ({ sidecarContainer }) => {
		...
		const result = await sidecarContainer.exec([
			"bash",
			"-c",
			["export DISPLAY=:99", "xhost 2>&1", "echo XHOST_PASS"].join("\n"),
		]);
...
test.describe
	.serial("ConfigureWindow stack_mode validation", () => {
		test.setTimeout(60_000);
		...
		test("valid stack modes (0-4) are accepted", async ({
			...
			const output = (
				await runPythonScript(
					sidecarContainer,
					"valid_stack_modes_0_4_are_accepted.py",
					{ env: { DISPLAY: ":99" } },
				)
			).output.trim();
			for (let i = 0; i < 5; i++) {
		...
		test("invalid stack mode (>4) returns BadValue error", async ({
			...
			const output = (
				await runPythonScript(
					sidecarContainer,
					"invalid_stack_mode_4_returns_badvalue_error.py",
					{ env: { DISPLAY: ":99" } },
				)
			).output.trim();
...
test.describe
	.serial("RotateProperties edge cases", () => {
		test("RotateProperties with duplicate atoms returns BadMatch", async ({
			...
			const output = (
				await runPythonScript(
					sidecarContainer,
					"rotateproperties_with_duplicate_atoms_returns_badmatch.py",
					{ env: { DISPLAY: ":99" } },
				)
			).output.trim();
