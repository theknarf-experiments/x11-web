...
test.describe("Server grab robustness", () => {
	test("server grab is released on client disconnect", async ({
		...
		const result = await runPythonScript(
			sidecarContainer,
			"server_grab_released_disconnect.py",
			{ env: { DISPLAY: ":99" } },
		);
...
test.describe("Resource cleanup on client disconnect", () => {
	test("windows are destroyed when client disconnects in Destroy mode", async ({
		...
		const result = await runPythonScript(
			sidecarContainer,
			"client_disconnect_destroy_windows.py",
			{ env: { DISPLAY: ":99" } },
		);
		console.log(`Destroy-mode test output: ${result.output}`);
		const match = result.output.match(/cleanup-destroy: pass=(\d+) fail=(\d+)/);
	...
	test("SetCloseDownMode RetainTemporary keeps windows alive", async ({
		...
		const result = await runPythonScript(
			sidecarContainer,
			"setclosedownmode_retaintemporary.py",
			{ env: { DISPLAY: ":99" } },
		);
		const match = result.output.match(/cleanup-retain: pass=(\d+) fail=(\d+)/);
		expect(match).toBeTruthy();
		expect(Number.parseInt(match![2], 10)).toBe(0);
		expect(Number.parseInt(match![1], 10)).toBeGreaterThanOrEqual(2);
	});
...
test.describe("Resource cleanup on disconnect", () => {
	test("server cleans up resources after client disconnect", async ({
		...
		const result = await runPythonScript(
			sidecarContainer,
			"resource_cleanup_after_disconnect.py",
			{ env: { DISPLAY: ":99" } },
		);
	...
	test("SaveSet reparenting works on WM disconnect", async ({
		...
		const result = await runPythonScript(
			sidecarContainer,
			"saveset_reparenting_wm_disconnect.py",
			{ env: { DISPLAY: ":99" } },
		);
...
test.describe
	.serial("python3-xlib edge cases", () => {
		test("SetCloseDownMode RetainPermanent preserves window", async ({
			...
			const output = (
				await runPythonScript(
					sidecarContainer,
					"setclosedownmode_retainpermanent_preserves_window.py",
					{ env: { DISPLAY: ":99" } },
				)
			).output.trim();
		...
		test("Window gravity during resize (NorthEast)", async ({
			...
			const output = (
				await runPythonScript(
					sidecarContainer,
					"window_gravity_during_resize_northeast.py",
					{ env: { DISPLAY: ":99" } },
				)
			).output.trim();
		...
		test("GrabServer serialization", async ({ sidecarContainer }) => {
			test.setTimeout(30_000);
			const output = (
				await runPythonScript(sidecarContainer, "grabserver_serialization.py", {
					env: { DISPLAY: ":99" },
				})
			).output.trim();
		...
		test("Exposure event delivery on map", async ({ sidecarContainer }) => {
			const output = (
				await runPythonScript(
					sidecarContainer,
					"exposure_event_delivery_on_map.py",
					{ env: { DISPLAY: ":99" } },
				)
			).output.trim();
		...
		test("PropertyNotify event on property change", async ({
			...
			const output = (
				await runPythonScript(
					sidecarContainer,
					"propertynotify_event_on_property_change.py",
					{ env: { DISPLAY: ":99" } },
				)
			).output.trim();
		...
		test("Selection protocol: SetSelectionOwner / ConvertSelection exchange", async ({
			...
			const output = (
				await runPythonScript(
					sidecarContainer,
					"selection_protocol_setselectionowner_convertselection_exchange.py",
					{ env: { DISPLAY: ":99" } },
				)
			).output.trim();
...
test.describe
	.serial("Advanced protocol compliance", () => {
		test("PutImage and GetImage round-trip at depth 24 (ZPixmap)", async ({
			...
			const output = (
				await runPythonScript(
					sidecarContainer,
					"putimage_and_getimage_round_trip_at_depth_24_zpixmap.py",
					{ env: { DISPLAY: ":99" } },
				)
			).output.trim();
		...
		test("PutImage Bitmap format (depth 1)", async ({ sidecarContainer }) => {
			const output = (
				await runPythonScript(
					sidecarContainer,
					"putimage_bitmap_format_depth_1.py",
					{ env: { DISPLAY: ":99" } },
				)
			).output.trim();
		...
		test("CreatePixmap and FreePixmap for all depths", async ({
			...
			const output = (
				await runPythonScript(
					sidecarContainer,
					"createpixmap_and_freepixmap_for_all_depths.py",
					{ env: { DISPLAY: ":99" } },
				)
			).output.trim();
		...
		test("Window border_width is reported in GetGeometry", async ({
			...
			const output = (
				await runPythonScript(
					sidecarContainer,
					"window_border_width_is_reported_in_getgeometry.py",
					{ env: { DISPLAY: ":99" } },
				)
			).output.trim();
		...
		test("Window gravity affects child positioning on resize", async ({
			...
			const output = (
				await runPythonScript(
					sidecarContainer,
					"window_gravity_affects_child_positioning_on_resize.py",
					{ env: { DISPLAY: ":99" } },
				)
			).output.trim();
		...
		test("SubstructureRedirectMask is exclusive (BadAccess)", async ({
			...
			const output = (
				await runPythonScript(
					sidecarContainer,
					"substructureredirectmask_is_exclusive_badaccess.py",
					{ env: { DISPLAY: ":99" } },
				)
			).output.trim();
		...
		test("SYNC extension counter/alarm operations", async ({
			...
			const output = (
				await runPythonScript(
					sidecarContainer,
					"sync_extension_counter_alarm_operations.py",
					{ env: { DISPLAY: ":99" } },
				)
			).output.trim();
		...
		test("Big-Requests extension enables large requests", async ({
			...
			const output = (
				await runPythonScript(
					sidecarContainer,
					"big_requests_extension_enables_large_requests.py",
					{ env: { DISPLAY: ":99" } },
				)
			).output.trim();
		...
		test("Stacking order changes with ConfigureWindow", async ({
			...
			const output = (
				await runPythonScript(
					sidecarContainer,
					"stacking_order_changes_with_configurewindow.py",
					{ env: { DISPLAY: ":99" } },
				)
			).output.trim();
		...
		test("CirculateWindow raises lowest and lowers highest", async ({
			...
			const output = (
				await runPythonScript(
					sidecarContainer,
					"circulatewindow_raises_lowest_and_lowers_highest.py",
					{ env: { DISPLAY: ":99" } },
				)
			).output.trim();
		...
		test("SetCloseDownMode RetainPermanent preserves resources", async ({
			...
			const output = (
				await runPythonScript(
					sidecarContainer,
					"setclosedownmode_retainpermanent_preserves_resources.py",
					{ env: { DISPLAY: ":99" } },
				)
			).output.trim();
		...
		test("GrabServer blocks other clients", async ({ sidecarContainer }) => {
			const output = (
				await runPythonScript(
					sidecarContainer,
					"grabserver_blocks_other_clients_2.py",
					{ env: { DISPLAY: ":99" } },
				)
			).output.trim();
		...
		test("MIT-SHM extension is functional", async ({ sidecarContainer }) => {
			const output = (
				await runPythonScript(
					sidecarContainer,
					"mit_shm_extension_is_functional.py",
					{ env: { DISPLAY: ":99" } },
				)
			).output.trim();
		...
		test("COMPOSITE extension is functional", async ({ sidecarContainer }) => {
			const output = (
				await runPythonScript(
					sidecarContainer,
					"composite_extension_is_functional.py",
					{ env: { DISPLAY: ":99" } },
				)
			).output.trim();
		...
		test("COMPOSITE RedirectWindow and NameWindowPixmap work", async ({
			...
			const output = (
				await runPythonScript(
					sidecarContainer,
					"composite_redirectwindow_and_namewindowpixmap_work.py",
					{ env: { DISPLAY: ":99" } },
				)
			).output.trim();
		...
		test("DAMAGE extension is functional and tracks regions", async ({
			...
			const output = (
				await runPythonScript(
					sidecarContainer,
					"damage_extension_is_functional_and_tracks_regions.py",
					{ env: { DISPLAY: ":99" } },
				)
			).output.trim();
		...
		test("Error handling: BadWindow for invalid window ID", async ({
			...
			const output = (
				await runPythonScript(
					sidecarContainer,
					"error_handling_badwindow_for_invalid_window_id.py",
					{ env: { DISPLAY: ":99" } },
				)
			).output.trim();
		...
		test("Error handling: BadValue for invalid arguments", async ({
			...
			const output = (
				await runPythonScript(
					sidecarContainer,
					"error_handling_badvalue_for_invalid_arguments.py",
					{ env: { DISPLAY: ":99" } },
				)
			).output.trim();
		...
		test("Multi-client event delivery via EventBroadcaster", async ({
			...
			const output = (
				await runPythonScript(
					sidecarContainer,
					"multi_client_event_delivery_via_eventbroadcaster.py",
					{ env: { DISPLAY: ":99" } },
				)
			).output.trim();
		...
		test("WarpPointer moves cursor position", async ({ sidecarContainer }) => {
			const output = (
				await runPythonScript(
					sidecarContainer,
					"warppointer_moves_cursor_position.py",
					{ env: { DISPLAY: ":99" } },
				)
			).output.trim();
		...
		test("CopyArea between windows works", async ({ sidecarContainer }) => {
			const output = (
				await runPythonScript(
					sidecarContainer,
					"copyarea_between_windows_works.py",
					{ env: { DISPLAY: ":99" } },
				)
			).output.trim();
		...
		test("RotateProperties works correctly", async ({ sidecarContainer }) => {
			const output = (
				await runPythonScript(
					sidecarContainer,
					"rotateproperties_works_correctly.py",
					{ env: { DISPLAY: ":99" } },
				)
			).output.trim();
		...
		test("KillClient destroys client resources", async ({
			...
			const output = (
				await runPythonScript(
					sidecarContainer,
					"killclient_destroys_client_resources.py",
					{ env: { DISPLAY: ":99" } },
				)
			).output.trim();
		...
		test("SetInputFocus and GetInputFocus round-trip", async ({
			...
			const output = (
				await runPythonScript(
					sidecarContainer,
					"setinputfocus_and_getinputfocus_round_trip.py",
					{ env: { DISPLAY: ":99" } },
				)
			).output.trim();
		...
		test("ListProperties returns all set properties", async ({
			...
			const output = (
				await runPythonScript(
					sidecarContainer,
					"listproperties_returns_all_set_properties.py",
					{ env: { DISPLAY: ":99" } },
				)
			).output.trim();
		...
		test("QueryBestSize returns valid tile/stipple sizes", async ({
			...
			const output = (
				await runPythonScript(
					sidecarContainer,
					"querybestsize_returns_valid_tile_stipple_sizes.py",
					{ env: { DISPLAY: ":99" } },
				)
			).output.trim();
		...
		test("glmark2 smoke test (GLX rendering)", async ({ sidecarContainer }) => {
		...
		test("XTEST FakeInput generates events", async ({ sidecarContainer }) => {
			const output = (
				await runPythonScript(
					sidecarContainer,
					"xtest_fakeinput_generates_events.py",
					{ env: { DISPLAY: ":99" } },
				)
			).output.trim();
		...
		test("RECORD extension is available", async ({ sidecarContainer }) => {
			const output = (
				await runPythonScript(
					sidecarContainer,
					"record_extension_is_available.py",
					{ env: { DISPLAY: ":99" } },
				)
			).output.trim();
		...
		test("DOUBLE-BUFFER extension is available", async ({
			...
			const output = (
				await runPythonScript(
					sidecarContainer,
					"double_buffer_extension_is_available.py",
					{ env: { DISPLAY: ":99" } },
				)
			).output.trim();
		...
		test("Present extension is available", async ({ sidecarContainer }) => {
			const output = (
				await runPythonScript(
					sidecarContainer,
					"present_extension_is_available.py",
					{ env: { DISPLAY: ":99" } },
				)
			).output.trim();
		...
		test("SECURITY extension is available", async ({ sidecarContainer }) => {
			const output = (
				await runPythonScript(
					sidecarContainer,
					"security_extension_is_available.py",
					{ env: { DISPLAY: ":99" } },
				)
			).output.trim();
		...
		test("XVideo extension is available", async ({ sidecarContainer }) => {
			const output = (
				await runPythonScript(
					sidecarContainer,
					"xvideo_extension_is_available.py",
					{ env: { DISPLAY: ":99" } },
				)
			).output.trim();
		...
		test("Backing store preserves window contents across unmap/remap", async ({
			...
			const output = (
				await runPythonScript(
					sidecarContainer,
					"backing_store_preserves_window_contents_across_unmap_remap.py",
					{ env: { DISPLAY: ":99" } },
				)
			).output.trim();
		...
		test("Bit gravity preserves content on resize", async ({
			...
			const output = (
				await runPythonScript(
					sidecarContainer,
					"bit_gravity_preserves_content_on_resize.py",
					{ env: { DISPLAY: ":99" } },
				)
			).output.trim();
		...
		test("PolyLine and PolySegment draw without errors", async ({
			...
			const output = (
				await runPythonScript(
					sidecarContainer,
					"polyline_and_polysegment_draw_without_errors.py",
					{ env: { DISPLAY: ":99" } },
				)
			).output.trim();
		...
		test("Selection protocol (clipboard) works end-to-end", async ({
			...
			const output = (
				await runPythonScript(
					sidecarContainer,
					"selection_protocol_clipboard_works_end_to_end.py",
					{ env: { DISPLAY: ":99" } },
				)
			).output.trim();
