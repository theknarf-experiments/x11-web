...
test.describe
	.serial("Grab protocol compliance", () => {
		test("GrabPointer succeeds on a viewable window", async ({
			...
			const output = (
				await runPythonScript(
					sidecarContainer,
					"grabpointer_succeeds_on_a_viewable_window.py",
					{ env: { DISPLAY: ":99" } },
				)
			).output.trim();
		...
		test("GrabPointer on unmapped window returns NotViewable", async ({
			...
			const output = (
				await runPythonScript(
					sidecarContainer,
					"grabpointer_on_unmapped_window_returns_notviewable.py",
					{ env: { DISPLAY: ":99" } },
				)
			).output.trim();
		...
		test("GrabKeyboard succeeds on a viewable window", async ({
			...
			const output = (
				await runPythonScript(
					sidecarContainer,
					"grabkeyboard_succeeds_on_a_viewable_window.py",
					{ env: { DISPLAY: ":99" } },
				)
			).output.trim();
		...
		test("GrabButton and passive activation via xdotool", async ({
			...
			const output = (
				await runPythonScript(
					sidecarContainer,
					"grabbutton_and_passive_activation_via_xdotool.py",
					{ env: { DISPLAY: ":99" } },
				)
			).output.trim();
		...
		test("GrabKey passive grab lifecycle", async ({ sidecarContainer }) => {
			const output = (
				await runPythonScript(
					sidecarContainer,
					"grabkey_passive_grab_lifecycle.py",
					{ env: { DISPLAY: ":99" } },
				)
			).output.trim();
...
test.describe
	.serial("Event delivery compliance", () => {
		test("EnterNotify and LeaveNotify on pointer warp with detail modes", async ({
			...
			const output = (
				await runPythonScript(
					sidecarContainer,
					"enternotify_and_leavenotify_on_pointer_warp_with_detail_modes.py",
					{ env: { DISPLAY: ":99" } },
				)
			).output.trim();
		...
		test("FocusIn and FocusOut events on SetInputFocus", async ({
			...
			const output = (
				await runPythonScript(
					sidecarContainer,
					"focusin_and_focusout_events_on_setinputfocus.py",
					{ env: { DISPLAY: ":99" } },
				)
			).output.trim();
		...
		test("GrabServer and UngrabServer complete successfully", async ({
			...
			const output = (
				await runPythonScript(
					sidecarContainer,
					"grabserver_and_ungrabserver_complete_successfully.py",
					{ env: { DISPLAY: ":99" } },
				)
			).output.trim();
		...
		test("AllowEvents modes complete without error", async ({
			...
			const output = (
				await runPythonScript(
					sidecarContainer,
					"allowevents_modes_complete_without_error.py",
					{ env: { DISPLAY: ":99" } },
				)
			).output.trim();
...
test.describe
	.serial("Core protocol edge cases (Phase 7)", () => {
		test("QueryTree returns correct parent-child relationships", async ({
			...
			const output = (
				await runPythonScript(
					sidecarContainer,
					"querytree_returns_correct_parent_child_relationships.py",
					{ env: { DISPLAY: ":99" } },
				)
			).output.trim();
		...
		test("GetGeometry returns correct window dimensions", async ({
			...
			const output = (
				await runPythonScript(
					sidecarContainer,
					"getgeometry_returns_correct_window_dimensions.py",
					{ env: { DISPLAY: ":99" } },
				)
			).output.trim();
		...
		test("InternAtom and GetAtomName round-trip", async ({
			...
			const output = (
				await runPythonScript(
					sidecarContainer,
					"internatom_and_getatomname_round_trip.py",
					{ env: { DISPLAY: ":99" } },
				)
			).output.trim();
		...
		test("ChangeProperty and GetProperty round-trip", async ({
			...
			const output = (
				await runPythonScript(
					sidecarContainer,
					"changeproperty_and_getproperty_round_trip.py",
					{ env: { DISPLAY: ":99" } },
				)
			).output.trim();
		...
		test("GrabServer/UngrabServer works", async ({ sidecarContainer }) => {
			const output = (
				await runPythonScript(
					sidecarContainer,
					"grabserver_ungrabserver_works.py",
					{ env: { DISPLAY: ":99" } },
				)
			).output.trim();
...
test.describe("Grab operations", () => {
	test("GrabPointer and UngrabPointer via xdotool", async ({
		...
		const result = await runPythonScript(
			sidecarContainer,
			"grabpointer_ungrabpointer_xdotool.py",
			{ env: { DISPLAY: ":99" } },
		);
		const match = result.output.match(/grabs-basic: pass=(\d+) fail=(\d+)/);
	...
	test("passive button grab and ungrab", async ({ sidecarContainer }) => {
		...
		const result = await runPythonScript(
			sidecarContainer,
			"passive_button_grab_ungrab.py",
			{ env: { DISPLAY: ":99" } },
		);
		const match = result.output.match(/grabs-passive: pass=(\d+) fail=(\d+)/);
		expect(match).toBeTruthy();
		expect(Number.parseInt(match![2], 10)).toBe(0);
		expect(Number.parseInt(match![1], 10)).toBeGreaterThanOrEqual(4);
	});
...
test.describe
	.serial("Grab operations validation", () => {
		...
		test("GrabButton and UngrabButton work correctly", async ({
			...
			const output = (
				await runPythonScript(
					sidecarContainer,
					"grabbutton_and_ungrabbutton_work_correctly.py",
					{ env: { DISPLAY: ":99" } },
				)
			).output.trim();
		...
		test("GrabKey and UngrabKey work correctly", async ({
			...
			const output = (
				await runPythonScript(
					sidecarContainer,
					"grabkey_and_ungrabkey_work_correctly.py",
					{ env: { DISPLAY: ":99" } },
				)
			).output.trim();
		...
		test("GrabKeyboard and UngrabKeyboard work correctly", async ({
			...
			const output = (
				await runPythonScript(
					sidecarContainer,
					"grabkeyboard_and_ungrabkeyboard_work_correctly.py",
					{ env: { DISPLAY: ":99" } },
				)
			).output.trim();
...
test.describe
	.serial("Passive grab cleanup on disconnect", () => {
		test("passive grabs are cleaned up when client disconnects", async ({
			...
			const output = (
				await runPythonScript(
					sidecarContainer,
					"passive_grabs_are_cleaned_up_when_client_disconnects.py",
					{ env: { DISPLAY: ":99" } },
				)
			).output.trim();
