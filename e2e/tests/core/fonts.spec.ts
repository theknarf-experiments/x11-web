...
test.describe
	.serial("Font system (Phase 7)", () => {
		test("'fixed' font can be opened", async ({ sidecarContainer }) => {
			const output = (
				await runPythonScript(sidecarContainer, "fixed_font_can_be_opened.py", {
					env: { DISPLAY: ":99" },
				})
			).output.trim();
		...
		test("QueryTextExtents returns valid metrics", async ({
			...
			const output = (
				await runPythonScript(
					sidecarContainer,
					"querytextextents_returns_valid_metrics.py",
					{ env: { DISPLAY: ":99" } },
				)
			).output.trim();
	...
	.serial("Font handling", () => {
		test("QueryFont returns valid metrics for fixed font", async ({
			...
			const output = (
				await runPythonScript(
					sidecarContainer,
					"queryfont_returns_valid_metrics_for_fixed_font.py",
					{ env: { DISPLAY: ":99" } },
				)
			).output.trim();
