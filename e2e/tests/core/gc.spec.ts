...
test.describe
	.serial("x11perf extended operations", () => {
		...
		test("x11perf arc and polygon operations", async ({ sidecarContainer }) => {
			const output = await execInSidecar(
				sidecarContainer,
...
test.describe("Conformance: x11perf extended validation", () => {
	test("x11perf drawing operations complete without crashes", async ({
		...
		const resultLines = result.output
			.split("\n")
			.filter((l: string) => l.includes("reps @") || l.includes("/sec"));
		expect(resultLines.length).toBeGreaterThanOrEqual(10);
	});
});

test.describe
	.serial("Wide dashed line rendering", () => {
		...
		test("wide dashed horizontal line creates visible gaps", async ({
			...
			const output = await probe(
				sidecarContainer,
				"wide_dashed_horizontal_line.py",
			);
		...
		test("GC function modes (copy, xor, invert) work", async ({
			...
			const output = (
				await runPythonScript(
					sidecarContainer,
					"gc_function_modes_copy_xor_invert_work.py",
					{ env: { DISPLAY: ":99" } },
				)
			).output.trim();
